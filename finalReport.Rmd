...
The collected data is related mainly to the detectors that located in _Sunnyside NB_, which includes these detectors: 1345, 1345, 1346, 1347, and 1348. Using the datasets and subsets we have, we can represents the volume in the period Sept. 15, 2011 through Nov. 15, 2011.
...
To get the _Peak Period Volume_, we can use the last result data to get the volume for 7:00am to 9:00am, and for 4:00pm to 6:00pm.
...
loops.79am.volume.avg <- loops.volume.intermed %>% filter(as.numeric(loops.volume.intermed$time) > 06 & as.numeric(loops.volume.intermed$time) < 10) %>% select(date, time, locationtext, volume) %>%  group_by(date, locationtext)  %>%  summarize(Avg.volume=mean(volume)) %>% arrange(date)
names(loops.79am.volume.avg)[1]<-"date"
names(loops.79am.volume.avg)[2]<-"locationtext"
names(loops.79am.volume.avg)[3]<-"Avg.volume"

#table4: This table is used to store the final result
#The mean/avg between 4:00 pm and 6:00 pm
loops.46pm.volume.avg <- loops.volume.intermed %>% filter(as.numeric(loops.volume.intermed$time) > 15 & as.numeric(loops.volume.intermed$time) < 19 ) %>% select(date, time, locationtext, volume) %>%  group_by(date, locationtext)  %>%  summarize(Avg.volume=mean(volume)) %>% arrange(date)
names(loops.46pm.volume.avg)[1]<-"date"
names(loops.46pm.volume.avg)[2]<-"locationtext"
names(loops.46pm.volume.avg)[3]<-"Avg.volume"

#table5: This table is used to store the final result
#The ds between 7:00 am and 9:00 am
loops.79am.volume.sd <- loops.volume.intermed %>% filter(as.numeric(loops.volume.intermed$time) > 06 & as.numeric(loops.volume.intermed$time) < 10) %>% select(date, time, locationtext, volume) %>%  group_by(date, locationtext)  %>%  summarize(sd.volume=sd(volume)) %>% arrange(date)
names(loops.79am.volume.sd)[1]<-"date"
names(loops.79am.volume.sd)[2]<-"locationtext"
names(loops.79am.volume.sd)[3]<-"sd.volume"


#table6: This table is used to store the final result
#The ds between 4:00 pm and 6:00 pm
loops.46pm.volume.sd <- loops.volume.intermed %>% filter(as.numeric(loops.volume.intermed$time) > 15 & as.numeric(loops.volume.intermed$time) < 19 ) %>% select(date, time, locationtext, volume) %>%  group_by(date, locationtext)  %>%  summarize(sd.volume=sd(volume)) %>% arrange(date)
names(loops.46pm.volume.sd)[1]<-"date"
names(loops.46pm.volume.sd)[2]<-"locationtext"
names(loops.46pm.volume.sd)[3]<-"sd.volume"
...
The first plot is for 7:00 am to 9:00 am, and the second one is for 4:00 pm to 6:00 pm.
...
It can clearly be seen that there is no major differences in the volume during the period mentioned above. 

The average volume for 7:00 am to 9:00 am is about 33.3% between 0 and 400, 33.3% between 400 and 800, and 33.3% between 800 and 1300. Moreover, the volume sometimes exceeds the 1300.

The average volume for 4:00 pm to 6:00 pm is about 33.3% between 0 and 700 and about 66.7% between 700 and 1400.



```{r plot, echo=FALSE, eval=FALSE}
#plots
plot(loops.79am.volume.avg$date, loops.79am.volume.avg$Avg.volume, main = "Sunnyside NB Volume - 7:00-9:00 am", xlab = "Date", ylab= "Avg(Volume)", col="blue")
plot(loops.46pm.volume.avg$date, loops.46pm.volume.avg$Avg.volume, main = "Sunnyside NB Volume - 4:00-6:00 pm", xlab = "Date", ylab= "Avg(Volume)", col="blue")
```
