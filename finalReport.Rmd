...
loops.79am.volume.sd <- loops.volume.intermed %>% filter(as.numeric(loops.volume.intermed$time) > 06 & as.numeric(loops.volume.intermed$time) < 10) %>% select(date, time, weekday, locationtext, volume) %>%  group_by(date, locationtext, weekday)  %>%  summarize(sd.volume=sd(volume)) %>% arrange(date)
...
loops.46pm.volume.sd <- loops.volume.intermed %>% filter(as.numeric(loops.volume.intermed$time) > 15 & as.numeric(loops.volume.intermed$time) < 19 ) %>% select(date, time, weekday, locationtext, volume) %>%  group_by(date, locationtext, weekday)  %>%  summarize(sd.volume=sd(volume)) %>% arrange(date)
...
#######
#####
I need to create 2 plots for each of the last tables (one for weekend and the other one for other date)
